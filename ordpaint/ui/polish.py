...
def _install_canvas_dynamics() -> None:
    ...
    def draw_segment(self, start, end) -> None:
        ...
        if self.tool.value == "eraser":
            painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_Clear)
        for index in range(count + 1):
            ratio = min(1.0, index / max(1, count))
...
def _add_brush_controls(window) -> None:
    ...
    for label, attr, minimum, maximum, default in controls:
        row = QHBoxLayout()
        row.addWidget(QLabel(label))
        slider = QSlider(Qt.Orientation.Horizontal)
        slider.setRange(minimum, maximum)
        slider.setValue(getattr(window.canvas, attr, default))
...
def install(window) -> None:
    ...
    for button in window.findChildren(QToolButton):
        ...
        if action is None:
            continue
        name = tool_icons.get(action.text())
        if name is None:
            continue
