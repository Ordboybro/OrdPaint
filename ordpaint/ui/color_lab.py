...
class ColorLabDock(QDockWidget):
    ...
    def __init__(self, window) -> None:
        ...
        self.v = QSpinBox(); self.v.setRange(0, 255)
        self.hsl_h = QSpinBox(); self.hsl_h.setRange(0, 359)
        self.hsl_s = QSpinBox(); self.hsl_s.setRange(0, 255)
        self.hsl_l = QSpinBox(); self.hsl_l.setRange(0, 255)
        for box in (self.h, self.s, self.v, self.hsl_h, self.hsl_s, self.hsl_l):
            box.valueChanged.connect(self._from_channels)
        form.addRow("HSV H", self.h); form.addRow("HSV S", self.s); form.addRow("HSV V", self.v)
        form.addRow("HSL H", self.hsl_h); form.addRow("HSL S", self.hsl_s); form.addRow("HSL L", self.hsl_l)
        layout.addLayout(form)
        colors = QHBoxLayout()
        self.fg = QPushButton("Передний"); self.bg = QPushButton("Фон")
        swap = QPushButton("⇄"); bw = QPushButton("B/W")
        ...
        swap.clicked.connect(self.swap_colors); bw.clicked.connect(self.reset_bw)
        for button in (self.fg, self.bg, swap, bw): colors.addWidget(button)
        layout.addLayout(colors)
        palette_grid = QGridLayout()
        self.palette_buttons = []
        for index in range(12):
            button = QPushButton(); button.setFixedSize(28, 28)
            button.clicked.connect(lambda _=False, i=index: self._use_palette(i))
            self.palette_buttons.append(button); palette_grid.addWidget(button, index // 6, index % 6)
        layout.addLayout(palette_grid)
        palette_buttons = QHBoxLayout()
        save = QPushButton("Сохранить палитру"); clear = QPushButton("Очистить палитру")
        save.clicked.connect(self.save_palette); clear.clicked.connect(self.clear_palette)
        palette_buttons.addWidget(save); palette_buttons.addWidget(clear); layout.addLayout(palette_buttons)
        layout.addStretch(1)
        self.setWidget(panel)
        self.setAllowedAreas(Qt.DockWidgetArea.LeftDockWidgetArea | Qt.DockWidgetArea.RightDockWidgetArea)
        self._palette = self._load_palette(); self._refresh_palette(); self._sync(self.foreground)
    ...
    def _persist_palette(self) -> None:
        self.settings.setValue(self.SETTINGS_KEY, json.dumps(self._palette)); self.settings.sync()
    ...
    def _sync(self, color: QColor) -> None:
        self._syncing = True
        h, s, v, _ = color.getHsv(); self.h.setValue(max(0, h)); self.s.setValue(s); self.v.setValue(v)
        hh, ss, ll, _ = color.getHsl(); self.hsl_h.setValue(max(0, hh)); self.hsl_s.setValue(ss); self.hsl_l.setValue(ll)
        self.fg.setStyleSheet(f"background: {self.foreground.name()};"); self.bg.setStyleSheet(f"background: {self.background.name()};")
        self._syncing = False
    ...
    def swap_colors(self) -> None:
        self.foreground, self.background = self.background, self.foreground; self._set_color(self.foreground)

    def reset_bw(self) -> None:
        self.foreground = QColor("#000000"); self.background = QColor("#ffffff"); self._set_color(self.foreground)

    def save_palette(self) -> None:
        value = self.foreground.name(QColor.NameFormat.HexArgb)
        if value not in self._palette: self._palette.insert(0, value)
        self._palette = self._palette[:12]; self._persist_palette(); self._refresh_palette()

    def clear_palette(self) -> None:
        self._palette.clear(); self._persist_palette(); self._refresh_palette()

    def _use_palette(self, index: int) -> None:
        if index < len(self._palette): self._set_color(QColor(self._palette[index]))
