...
class MainWindow(QMainWindow):
    ...
    def _create_actions(self) -> None:
        ...
        labels = {Tool.BRUSH: "Кисть", Tool.ERASER: "Ластик", Tool.LINE: "Линия", Tool.RECTANGLE: "Прямоугольник", Tool.ELLIPSE: "Эллипс", Tool.FILL: "Заливка", Tool.EYEDROPPER: "Пипетка", Tool.SELECT_RECT: "Выделение"}
        shortcuts = {Tool.BRUSH: "B", Tool.ERASER: "E", Tool.LINE: "L", Tool.RECTANGLE: "R", Tool.ELLIPSE: "O", Tool.FILL: "G", Tool.EYEDROPPER: "I", Tool.SELECT_RECT: "M"}
    ...
    def _create_toolbars(self) -> None:
        ...
        for tool, text in [(Tool.BRUSH, "Кисть"), (Tool.ERASER, "Ластик"), (Tool.LINE, "Линия"), (Tool.RECTANGLE, "Прям."), (Tool.ELLIPSE, "Эллипс"), (Tool.FILL, "Заливка"), (Tool.EYEDROPPER, "Пипетка"), (Tool.SELECT_RECT, "Выдел.")]:
        ...
        self.size_spin = QSpinBox(); self.size_spin.setRange(1, 500); self.size_spin.setValue(self.canvas.brush_size); self.size_spin.valueChanged.connect(self.canvas.set_brush_size); options.addWidget(self.size_spin)
        options.addWidget(QLabel("  Непрозрачность: "))
        self.opacity_slider = QSlider(Qt.Orientation.Horizontal); self.opacity_slider.setRange(1, 100); self.opacity_slider.setValue(100); self.opacity_slider.setFixedWidth(140); self.opacity_slider.valueChanged.connect(self.canvas.set_opacity); options.addWidget(self.opacity_slider)
        self.color_button = QPushButton("#111111"); self.color_button.clicked.connect(self.choose_color); options.addWidget(self.color_button)
        self._update_color_button(self.canvas.color)
        options.addSeparator(); self.zoom_label = QLabel("100%"); options.addWidget(self.zoom_label)

    def _create_layers_dock(self) -> None:
        dock = QDockWidget("Слои", self); dock.setObjectName("layersDock"); dock.setMinimumWidth(250)
        widget = QWidget(); layout = QVBoxLayout(widget); layout.setContentsMargins(8, 8, 8, 8)
        self.layers_list = QListWidget(); self.layers_list.currentRowChanged.connect(self._set_active_layer); self.layers_list.itemChanged.connect(self._change_layer_visibility); layout.addWidget(self.layers_list)
        row = QHBoxLayout()
        for text, slot in [("+", self.add_layer), ("⧉", self.duplicate_layer), ("−", self.remove_layer)]:
            button = QPushButton(text); button.setFixedWidth(60); button.clicked.connect(slot); row.addWidget(button)
    ...
    def _create_color_dock(self) -> None:
        dock = QDockWidget("Цвет", self); widget = QWidget(); layout = QVBoxLayout(widget)
        self.foreground_preview = QPushButton(); self.foreground_preview.setFixedHeight(44); self.foreground_preview.clicked.connect(self.choose_color); layout.addWidget(self.foreground_preview)
        self.red_slider = QSlider(Qt.Orientation.Horizontal); self.green_slider = QSlider(Qt.Orientation.Horizontal); self.blue_slider = QSlider(Qt.Orientation.Horizontal)
        for slider in (self.red_slider, self.green_slider, self.blue_slider):
            slider.setRange(0, 255); slider.valueChanged.connect(self._sliders_to_color); layout.addWidget(slider)
    ...
    def _on_document_changed(self) -> None:
        self._push_history(); self._refresh_layers(); self.canvas.update(); self.setWindowTitle("OrdPaint — *")
    ...
    def _update_history_actions(self) -> None:
        if hasattr(self, "undo_action"):
            self.undo_action.setEnabled(self.history.can_undo()); self.redo_action.setEnabled(self.history.can_redo())
    ...
    def _refresh_layers(self) -> None:
        ...
        for layer in reversed(self.document.layers):
            item = QListWidgetItem(layer.name); item.setCheckState(Qt.CheckState.Checked if layer.visible else Qt.CheckState.Unchecked); self.layers_list.addItem(item)
    ...
    def _change_layer_visibility(self, item: QListWidgetItem) -> None:
        row = self.layers_list.row(item); index = len(self.document.layers) - 1 - row
        self._push_history(); self.document.layers[index].visible = item.checkState() == Qt.CheckState.Checked; self.canvas.update()

    def add_layer(self) -> None:
        self._push_history(); self.document.add_layer(); self._refresh_layers(); self.canvas.update()

    def duplicate_layer(self) -> None:
        self._push_history(); self.document.duplicate_active_layer(); self._refresh_layers(); self.canvas.update()

    def remove_layer(self) -> None:
        if len(self.document.layers) > 1:
            self._push_history(); self.document.remove_active_layer(); self._refresh_layers(); self.canvas.update()
    ...
    def _set_color_from_canvas(self, color: QColor) -> None:
        self.canvas.set_color(color); self._sync_color_sliders(color); self._update_color_button(color)
    ...
    def _update_color_button(self, color: QColor) -> None:
        text_color = "#ffffff" if color.lightness() < 128 else "#111111"; self.color_button.setText(color.name()); self.color_button.setStyleSheet(f"background:{color.name()}; color:{text_color};")

    def new_document(self) -> None:
        self.history.clear(); self.current_path = None; self._replace_document(Document()); self.setWindowTitle("OrdPaint — Untitled")
    ...
    def _style_sheet() -> str:
        return """
        QMainWindow { background: #1e1f22; color: #d7d7d7; }
        QMenuBar, QMenu, QStatusBar { background: #26272b; color: #d7d7d7; }
        QMenu::item:selected { background: #3c4048; }
        QToolBar { background: #2a2b30; border: none; spacing: 4px; padding: 4px; }
        QToolButton { color: #d7d7d7; background: transparent; border: 1px solid transparent; border-radius: 6px; padding: 5px; }
        QToolButton:hover, QToolButton:checked { background: #3d4f66; border-color: #5d83ad; }
        QDockWidget::title { background: #2a2b30; padding: 7px; font-weight: 700; }
        QListWidget { background: #25262a; border: 1px solid #3c3d42; color: #dedede; }
        QListWidget::item:selected { background: #394d64; }
        QPushButton, QSpinBox { background: #34363c; border: 1px solid #4b4d54; border-radius: 5px; color: #eeeeee; padding: 6px 9px; }
        QPushButton:hover { background: #44474f; }
        QSlider::groove:horizontal { height: 4px; background: #4b4d54; border-radius: 2px; }
        QSlider::handle:horizontal { width: 12px; margin: -4px 0; border-radius: 6px; background: #63a4ff; }
        """
