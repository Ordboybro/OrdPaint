...
class Canvas(QWidget):
    ...
    def fit_to_window(self) -> None:
        if self.document.width <= 0 or self.document.height <= 0:
            return
        scale = min((self.width() - 80) / self.document.width, (self.height() - 80) / self.document.height)
        self.set_zoom(scale)

    def _image_top_left(self) -> QPointF:
        scaled_w = self.document.width * self.zoom
        scaled_h = self.document.height * self.zoom
        return QPointF((self.width() - scaled_w) / 2 + self.pan.x(), (self.height() - scaled_h) / 2 + self.pan.y())
    ...
    def _normalized_rect(self, start: QPoint, end: QPoint) -> QRect:
    ...
    def paintEvent(self, event) -> None:
        ...
        if self._drawing and self._start_canvas_pos and self._last_canvas_pos and self.tool in {Tool.LINE, Tool.RECTANGLE, Tool.ELLIPSE, Tool.SELECT_RECT}:
            pen = QPen(QColor("#63a4ff") if self.tool == Tool.SELECT_RECT else self._paint_color(), max(1, self.brush_size), Qt.PenStyle.DashLine if self.tool == Tool.SELECT_RECT else Qt.PenStyle.SolidLine)
            painter.setPen(pen)
            painter.setBrush(Qt.BrushStyle.NoBrush)
            if self.tool == Tool.LINE:
                painter.drawLine(self._start_canvas_pos, self._last_canvas_pos)
            else:
                painter.drawRect(self._normalized_rect(self._start_canvas_pos, self._last_canvas_pos)) if self.tool in {Tool.RECTANGLE, Tool.SELECT_RECT} else painter.drawEllipse(self._normalized_rect(self._start_canvas_pos, self._last_canvas_pos))
        if self.selection_rect:
            painter.setPen(QPen(QColor("#63a4ff"), 1, Qt.PenStyle.DashLine))
            painter.setBrush(Qt.BrushStyle.NoBrush)
    ...
    def mousePressEvent(self, event) -> None:
        ...
        if self.tool == Tool.EYEDROPPER:
            color = self.document.composite().toImage().pixelColor(point)
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        if self._start_canvas_pos and self._last_canvas_pos:
            if self.tool in {Tool.LINE, Tool.RECTANGLE, Tool.ELLIPSE}:
                ...
                self.selection_rect = self._normalized_rect(self._start_canvas_pos, self._last_canvas_pos)
        self._finish_action()
